...
def load_model():
    ...
    try:
        ...
        model.load_model('best_model_Clinical_iTED.json')  # Try root directory first
        return model, True
    except:
        try:
            model = xgb.XGBClassifier()
            model.load_model('models/best_model_Clinical_iTED.json')  # Try models folder
            return model, True
        except:
            # If loading fails, return None (demo mode)
            return None, False
